...
static int should_copy(int ic, int oc)
{
    ...
    case AV_CODEC_ID_MP3:
        return (!of || strcmp(of->ctx->oformat->name, "matroska"));
    case AV_CODEC_ID_H264:
        return 1;
    case AV_CODEC_ID_OPUS:
    case AV_CODEC_ID_VP8:
        return (of && (!strcmp(of->ctx->oformat->name, "matroska") || !strcmp(of->ctx->oformat->name, "webm")));
    }
    return 0;
}

static int choose_encoder(OptionsContext *o, AVFormatContext *s, InputStream *ist, OutputStream *ost)
{
    ...
    if (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO || type == AVMEDIA_TYPE_SUBTITLE) {
        ...
        if (!codec_name) {           
            int codec_id = av_guess_codec(s->oformat, NULL, s->url,
                                          NULL, ost->st->codecpar->codec_type);
            if (should_copy(ist->dec_ctx->codec_id, codec_id)) {
                ost->stream_copy = 1;
            } else {
            ...
            AVCodec *codec = find_codec_or_die(codec_name, ost->st->codecpar->codec_type, 1);
            if (should_copy(ist->dec_ctx->codec_id, codec->id)) {
                ost->stream_copy = 1;
            } else {
                ost->enc = codec;
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
    ...
    ret = choose_encoder(o, oc, ist, ost);
    if (ret < 0) {
